...
---------------------------------------------- Link for the challenge: https://codeforces.com/problemset/problem/586/A -------------------------------------------

Alena has successfully passed the entrance exams to the university and is now looking forward to start studying.

One two-hour lesson at the Russian university is traditionally called a pair, it lasts for two academic hours (an academic hour is equal to 45 minutes).

The University works in such a way that every day it holds exactly n lessons. 
Depending on the schedule of a particular group of students, on a given day, some pairs may actually contain classes, but some may be empty (such pairs are called breaks).
...
# If there are no '1's, the counter is 0
if not one_indices:
    print(0)
else:
    counter = len(one_indices)
    # Check the gaps between consecutive '1's
    for i in range(len(one_indices) - 1):
        gap_size = one_indices[i+1] - one_indices[i] - 1       
        # If there is exactly one '0' between them, increase counter
        if gap_size == 1:
            counter += 1
    print(counter)