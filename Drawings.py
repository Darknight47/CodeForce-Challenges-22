...
------------------------------------------------ Link for the challenge: https://codeforces.com/problemset/problem/2244/A --------------------------------------------

During a geometry lesson, Iskander got very bored, so he decided to draw in Yura's notebook. To do this, he took a row and drew horizontal lines on it. 
Some lines are long, some are short, and some parts of the page remain empty.

The page is represented by a string s, where the character '*' denotes an empty part of the paper, and the character '#' denotes one centimeter of a drawn line. 
...
for _ in range(cases):
    n = int(input())
    s = input()
    longest_substring = max(s.split('*'), key=len)
    print(math.ceil(len(longest_substring) / 2) )