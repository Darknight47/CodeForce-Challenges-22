...
--------------------------------------------------- Link for the challenge: https://codeforces.com/problemset/problem/1721/B -----------------------------------
...
The cell (sx,sy) contains a deadly laser. If the robot comes into some cell that has distance less than or equal to d to the laser, 
it gets evaporated. The distance between two cells (x1,y1) and (x2,y2) is |x1−x2|+|y1−y2|.
...
The laser is neither in the starting cell, nor in the ending cell ((sx,sy)≠(1,1) and (sx,sy)≠(n,m)).
The starting cell (1,1) always has distance greater than d to the laser (|sx−1|+|sy−1|>d).

Output
For each testcase, print a single integer. If it's possible to reach the cell (n,m) from (1,1) 
...
for _ in range(cases):
    ...
    min_dist_bottom = min(y - 1, abs(n - x))
    min_dist_right = min(x - 1, abs(m - y))

    bottom_safe = min_dist_bottom > d
    right_safe = min_dist_right > d

    if(not bottom_safe and not right_safe):
        print("-1")
    else:
        print(n + m - 2)